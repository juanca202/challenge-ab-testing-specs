...
| **Rama** | `develop` (MFE y host al 2026-06-18) |
| **Fecha** | 2026-06-18 |
...
La US-29912 tiene trazabilidad sólida entre criterios `BR-XX`/`SC-XX`, casos ADO (`TC-29935`–`TC-29953`) y artefactos automatizados en `challenge-ab-testing` (MFE), `challenge-ab-testing-host` (contenedor) y la suite E2E Playwright `e2e/us-29912.spec.ts`. Se ejecutaron 134 tests unitarios/integración del MFE (100 % paso), 29 tests E2E de US-29912 (100 % paso; 3 omitidos por viewport), y cobertura del MFE al 100 % de ramas. **BR-07** queda parcial: scripts de despliegue S3/CloudFront documentados y cobertura verificada, pero SonarQube/análisis de seguridad no son ejecutables desde el repo en esta validación.
...
|------|----------|------------|
| Unit | Sí | **MFE** (`challenge-ab-testing`): 28 archivos en `src/features/seguro-embebido/`, `src/lib/api/` — p. ej. `VariantA.test.tsx`, `SeguroCtaBar.test.tsx`, `useVariantAssignment.test.tsx`, `consultarProducto.test.ts`. **Host**: 32 archivos — p. ej. `RemoteSeguroEmbebido.test.tsx`, `useInsuranceDecision.test.tsx`, `OnboardingLayout.test.tsx` |
...
| **Comando ejecutado** | `cd challenge-ab-testing && npm run test:run` · `npm run test:coverage` · `cd challenge-ab-testing-host && npm run test:run` · `npm run test:e2e -- e2e/us-29912.spec.ts` |
| **Resultado global** | MFE: 134/134 pasaron. Cobertura MFE: 100 % statements, 100 % branches. Host unit: 100/101 pasaron (1 fallo en `OnboardingLayout.test.tsx` — clase CSS `min-h-screen` vs `min-h-svh`, no vinculado a criterio BR/SC). E2E US-29912: 29 pasaron, 3 omitidos (viewport), 0 fallos |
...
- **Host — test fallido:** `OnboardingLayout.test.tsx` › `should_apply_responsive_layout_classes` falla por drift de clases Tailwind; no bloquea criterios BR/SC de US-29912 (TC-29944 E2E pasó).
